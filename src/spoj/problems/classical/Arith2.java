...
public class Arith2 {
	public static void main(String[] args) throws Exception {
		try {
			...
			int n=in.nextInt();
			long[] finalRes = new long[n];
			while(n-- > 0) {
				String[] x=in.nextLine().split(" ");
				long res = Long.parseLong(x[0]);
				for (int j = 1; j < x.length - 1; j++) {
					if(x[j].equals("+")) {
						res += Long.parseLong(x[j+1]);
					} else if(x[j].equals("-")) {
						res -= Long.parseLong(x[j+1]);
					} else if(x[j].equals("*")) {
						res *= Long.parseLong(x[j+1]);
					} else {
						res = res / Long.parseLong(x[j+1]);
					}
					j++;
				}
				finalRes[n]=res;
			}
			for (int i = finalRes.length; i > 0; i--) {
				out.printLine(finalRes[i-1]);
				out.flush();
			}
			out.close();
		} catch (Exception e) {

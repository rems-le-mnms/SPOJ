...
==========================================================
Author	: 		Rémi Kaeffer
Description : 	http://www.spoj.com/problems/SBANK/
==========================================================
STATUS : AC in 0.95s
 */
...
public class Sbank {
	public static void main(String[] args) throws Exception {
		...
		ArrayList<Map<String,Integer>> finalRes = new ArrayList<Map<String,Integer>>();
		int n= in.nextInt();
		while(n-- > 0) {
			...
			while(t-- > 0) {
				...
				if(res.get(res.size()-1).containsKey(numberCard)) {
					res.get(res.size()-1).put(numberCard, res.get(res.size()-1).get(numberCard) +1);
				} else {
					res.get(res.size()-1).put(numberCard, 1);
				}
			}
			finalRes.add(new TreeMap<String,Integer>(res.get(res.size()-1)));
		}
		for (int i = 0; i < finalRes.size(); i++) {
			for (Map.Entry entry : finalRes.get(i).entrySet()) {

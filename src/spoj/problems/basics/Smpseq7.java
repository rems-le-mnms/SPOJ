...
==========================================================
Author	: 		Rémi Kaeffer
Description : 	http://www.spoj.com/problems/SMPCPH1/
==========================================================
STATUS : ???
 */
...
public class Smpseq7 {
	public static void main(String[] args) throws Exception {
		...
		int cursor = 0;
		boolean cursorStatus = true;
		while(cursorStatus) {
			if(si.get(cursor)>=si.get(cursor+1) || cursor < si.size()) {
				cursorStatus=false;
			}
			cursor++;
		}
		while(cursor<si.size()-1) {
			if(si.get(cursor)<si.get(cursor+1)) {
				asc=false;
			}
			cursor++;
		}

...
==========================================================
Author	: 		Rémi Kaeffer
Description : 	http://www.spoj.com/problems/SMPCPH/
==========================================================
STATUS : AC in 0.18s
 */
		...
		while(t-- > 0) {
			...
			for (int i = 0; i < toEncode.length(); i++) {
				...
				for (int j = 0; j < input.length; j++) {
					if(toEncode.charAt(i)==input[j]) {
						newChar=j;
					}
				}
				if(newChar == -1) {
					encode.append(toEncode.charAt(i));
				} else {
					encode.append(output[newChar]);
				}
			}
			res.add(encode.toString());	
		}
